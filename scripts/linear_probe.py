"""
Linear Probe for ICH Detection using NeuroVFM features.
...
def load_features(features_dir, patient_ids, pooling="mean"):
    ...
    Args:
        features_dir: Directory containing {pid:03d}.pt files
        patient_ids: Array of patient IDs to load
        pooling: "mean", "max", or "mean_max"

    Returns:
        np.ndarray of shape [N_patients, D], valid_mask boolean array
    """
...
def run_cv(X, y, n_splits=NUM_CV_FOLDS, C_values=None):
    """
    Run stratified k-fold CV with nested hyperparameter tuning.
    """
    if C_values is None:
    ...
    for fold_idx, (train_idx, test_idx) in enumerate(skf.split(X, y)):
        ...
        for C in C_values:
            ...
            for inner_train, inner_val in inner_skf.split(X_train, y_train):
                ...
                prob = clf.predict_proba(X_train[inner_val])[:, 1]
                try:
        ...
        clf = LogisticRegression(
            C=best_C, max_iter=10000, random_state=SEED, solver="lbfgs"
        )
...
def run_loo(X, y):
    ...
    for train_idx, test_idx in loo.split(X):
        ...
        clf = LogisticRegression(
            C=1.0, max_iter=10000, random_state=SEED, solver="lbfgs"
        )
        clf.fit(X_train, y_train)
...
def plot_roc(roc_data, results_dir):
    ...
    ax.set_title("ROC - NeuroVFM Linear Probe for ICH Detection")
    ax.legend(loc="lower right")
    ax.set_xlim([-0.02, 1.02])
    ax.set_ylim([-0.02, 1.02])

    out_path = Path(results_dir) / "roc_curve.png"
...
def plot_confusion_matrix(y_true, y_pred, results_dir):
    ...
    ax.set_title("Confusion Matrix (aggregated across folds)")
    fig.colorbar(im)
    ...
    out_path = Path(results_dir) / "confusion_matrix.png"
    fig.savefig(out_path, dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def main():
    parser = get_base_parser("Linear Probe for ICH detection")
    ...
    parser.add_argument("--run-ablations", action="store_true",
                        help="Run pooling ablations (mean, max, mean_max)")
    ...
    print(f"\n{'='*50}")
    print(f"Stratified {NUM_CV_FOLDS}-Fold Cross-Validation (pooling={args.pooling})")
    print(f"{'='*50}")
    ...
    if args.run_ablations:
        print(f"\n{'='*50}")
        print("Pooling Ablations")
        print(f"{'='*50}")
        for pooling in ["mean", "max", "mean_max"]:
            print(f"\n--- {pooling} pooling ---")
            X_abl, mask_abl = load_features(
                args.features_dir, patient_ids, pooling=pooling
            )
            res = run_cv(X_abl, labels)
            ablation_results[pooling] = res["summary"]
            auc_m = res["summary"]["auc_mean"]
            auc_s = res["summary"]["auc_std"]
    ...
    if ablation_results:
        output["ablations"] = ablation_results
